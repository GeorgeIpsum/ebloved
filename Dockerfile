...
COPY --from=builder --chown=65534:65534 /app/_build/${MIX_ENV}/rel/ebloved ./

USER 65534:65534

EXPOSE 4000

# Run pending migrations (Ebloved.Release.migrate/0, via rel/overlays/bin/migrate)
# then start the server (rel/overlays/bin/server sets PHX_SERVER=true and execs
# `ebloved start`). This is the entrypoint Task 7's k8s deployment depends on.
ENTRYPOINT ["sh", "-c", "/app/bin/migrate && exec /app/bin/server"]
